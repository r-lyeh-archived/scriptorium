...
static sexp sexp_get_sha_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg2) {
  sexp res;
...
static sexp sexp_add_sha_data_x_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg2, sexp arg3) {
  sexp res;
  if (! (sexp_pointerp(arg2) && (sexp_pointer_tag(arg2) == sexp_unbox_fixnum(sexp_opcode_arg3_type(self)))))
    return sexp_type_exception(ctx, self, sexp_unbox_fixnum(sexp_opcode_arg3_type(self)), arg2);
...
static sexp sexp_start_sha_stub (sexp ctx, sexp self, sexp_sint_t n, sexp arg2) {
  sexp res;
...
sexp sexp_init_library (sexp ctx, sexp self, sexp_sint_t n, sexp env, const char* version, const sexp_abi_identifier_t abi) {
  ...
  op = sexp_define_foreign(ctx, env, "get-sha", 1, (sexp_proc1)sexp_get_sha_stub);
  if (sexp_opcodep(op)) {
  ...
  op = sexp_define_foreign(ctx, env, "add-sha-data!", 2, (sexp_proc1)sexp_add_sha_data_x_stub);
  if (sexp_opcodep(op)) {
    sexp_opcode_return_type(op) = sexp_make_fixnum(SEXP_OBJECT);
    sexp_opcode_arg3_type(op) = sexp_make_fixnum(sexp_type_tag(sexp_sha_context_type_obj));
  }
  op = sexp_define_foreign(ctx, env, "start-sha", 1, (sexp_proc1)sexp_start_sha_stub);
  if (sexp_opcodep(op)) {
    sexp_opcode_return_type(op) = sexp_make_fixnum(SEXP_OBJECT);
